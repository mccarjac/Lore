...
import { GitHubSection } from '@/datastores/github';
...
const renderSection = () =>
  render(
    <GitHubSection
      ctx={createDataStoreContext(genericRuleset)}
...
describe('GitHubSection', () => {
  ...
  it('offers to sync first or export anyway when the remote has moved', async () => {
    ...
    // The ruleset arrives on the context, not from a module-level singleton —
    // the PR body names the ruleset the data was exported from.
    await waitFor(() => {
