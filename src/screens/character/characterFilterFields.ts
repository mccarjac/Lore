...
export function useCharacterFilterFields(): FilterFieldConfig[] {
  ...
  return useMemo(() => {
    ...
    // `retiredStatus` is the one field left hand-written here: `retired` is a
    // builtin `GameCharacter` field, not a facet (unlike presence, which #56
    // moved onto a ruleset-declared `single` collection and so needs no
    // special case any more — it's generated by the loop above like any
    // other facet).
    fields.push({
      key: 'retiredStatus',
      type: 'select',
